...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_class):
      if j == y[i]:
        dW[j,:] += (np.exp(score_i[y[i]]/exp_score_total_i)*(-X_i.T + (exp_score_i[j] / exp_score_total_i) * X_i.T))
      else:
        dW[j,:] += (np.exp(score_i[y[i]]/exp_score_total_i)*(exp_score_i[j] / exp_score_total_i) * (X_i.T))
